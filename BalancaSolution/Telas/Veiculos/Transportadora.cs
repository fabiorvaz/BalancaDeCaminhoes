...
{
    public partial class Transportadora : Form
    {
        bool pesquisa = false;
        string tabela = "Transportadora";
        string nome_antigo = "";
        ...
        private void Transportadora_Load(object sender, EventArgs e)
        {
            Carregar_ListView();
        }
        ...
        private bool Validar_Dados(Button buton, ErrorProvider errorProvider)
        {
            ...
            {
                errorProvider.SetError(txt_nome, "Transportadora já cadastrado.");
                return false;
            }
        ...
        private void Transportadora_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Escape:
            ...
            try
            {
                ...
                if (pesquisa)
                {
                    ...
                    if (Properties.Settings.Default.Modo_Log)
                        Lib.Log.Log.gravarMenssagemDataHora("Transportadora " + nome_antigo + " foi atualizado.");
                }
                else
                {
                    if (!Validar_Dados(btn_salvar, errorProvider1)) return;
                    ...
                    if (Properties.Settings.Default.Modo_Log)
                        Lib.Log.Log.gravarMenssagemDataHora("Transportadora " + txt_nome.Text + " foi criado.");
                }
                Carregar_ListView();
                Limpar_Campos();
            }
